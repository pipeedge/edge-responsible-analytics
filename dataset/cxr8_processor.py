...
def load_chestxray8_data(max_samples=1000):  # Limit samples for IoT devices
    try:
        ...
        for sample in ds_subset:
            ...
            data['age'].append(sample['Patient Age'])
            data['gender'].append(sample['Patient Gender'])
            data['view_position'].append(sample['View Position'])
            data['patient_id'].append(sample['Patient ID'])
        ...
        logger.info(f"Loaded {len(df)} samples from CXR8 dataset")
        return df
        
    except Exception as e:
        logger.error(f"Error loading CXR8 data: {e}")
        raise
...
def process_cxr8_data(batch_size=16, max_samples=1000):
    ...
    try:
        ...
        df = load_chestxray8_data(max_samples=max_samples)
        ...
        logger.error(f"Error processing CXR8 data: {e}")
        raise
