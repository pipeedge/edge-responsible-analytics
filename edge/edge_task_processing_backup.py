...
MQTT_BROKER = os.getenv('MQTT_BROKER', '10.210.32.158')
...
def task_processing(task_type, model_type):
    ...
    # Wait for the next cycle or handle model updates
    # For example, wait for a certain time or until a model update is received
    # time.sleep(10)  # Adjust the sleep time as needed

# Route to upload model
@app.route('/upload_model', methods=['POST'])  # {{ edit_3 }}
def upload_model():
    data = request.json
    model_path = data.get('model_path')
    model_type = data.get('model_type')
    send_trained_model(model_path, model_type)  # Call existing function
    return jsonify({"status": "Model uploaded successfully"}), 200  # {{ edit_4 }}
...
if __name__ == "__main__":
    app.run(host='10.210.32.158', port=5000)
    model_type = 'MobileNet'
    task_type = 'inference'
    # Start task processing in the main thread
    task_processing(task_type, model_type)
